...
class TestResize < Minitest::Test
  ...
  def test_resize_fill
    # 200x100 image fill 100x100 -> scale to 200x100 at 2x, then crop to 100x100
    image = create_test_image(200, 100)
  ...
  def test_resize_preserves_color
    # Create solid red image, resize should keep it red
    pixels = "\xFF\x00\x00".b * (64 * 64)
    image = Pura::Jpeg::Image.new(64, 64, pixels)
  ...
  def test_resize_and_encode_roundtrip
    ...
    resized = image.resize(32, 32)
    out_path = File.join(TMP_DIR, "test_resized.jpg")
    Pura::Jpeg.encode(resized, out_path, quality: 90)
